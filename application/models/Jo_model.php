...
class Jo_model extends CI_Model
{
	...
	function get_list_new_join($postData = null)
	{
		...
		$dbjo = $this->load->database('dbjo', TRUE);
		...
		if ($project != "0") {
			...
			if ($searchValue != '') {
				if (strlen($searchValue) >= 3) {
					$searchQuery = " (emp.employee_id like '%" . $searchValue .  "%' or emp.first_name like '%" . $searchValue . "%' or pos.designation_name like '%" . $searchValue . "%' or emp.penempatan like '%" . $searchValue . "%') ";
				}
			}
			...
			if (($project != null) && ($project != "") && ($project != '0')) {
				$filterProject = "emp.project_id = '" . $project . "'";
			} else {
				$filterProject = "";
			}
			...
			if (($sdate != null) && ($edate != "")) {
				...
				$filterPeriode = "DATE_FORMAT(emp.date_of_joining, '%Y-%m-%d') BETWEEN '".$sdate."' AND '".$edate."'";
			...
			## Fetch records
			// $this->db->select('*');
			$this->db->select('emp.user_id');
			$this->db->select('emp.employee_id');
			$this->db->select('emp.first_name');
			$this->db->select('emp.project_id');
			$this->db->select('pro.title');
			$this->db->select('emp.designation_id');
			$this->db->select('pos.designation_name');
			$this->db->select('emp.penempatan');
			$this->db->select('emp.date_of_joining');
			$this->db->select('emp.id_screening');
			// $this->db->select('tx_cio.date_cio');
			// $this->db->select('tx_cio.datetimephone_in');
			// $this->db->select('tx_cio.latitude_in');
			// $this->db->select('tx_cio.datetimephone_out');
			// $dbtraxes->where($kondisiDefaultQuery);
			if ($searchQuery != '') {
				$this->db->where($searchQuery);
			}
			...
			foreach ($records as $record) {
				...
				//cek interviewer
				// $nik_validation = "0";
				$nik_validation_query = $this->Employees_model->get_valiadation_status($record->id_screening);
	...
	public function get_jo_interviewer($id)
	{
		// $api_traxes_db = $this->load->database('api_traxes_db', TRUE);

		$dbjo = $this->load->database('dbjo', TRUE);
		// $sql = 'SELECT * FROM xin_user_mobile WHERE employee_id = ?';
		$sql = "SELECT interview_rto_by, interview_rto_by_name FROM screening WHERE id = ?;";
		$binds = array($id);
		$query = $dbjo->query($sql, $binds);
