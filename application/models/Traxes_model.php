...
class Traxes_model extends CI_Model
{
	...
	* @author Fadla Qamara
	*/
	function get_list_summary($postData = null)
	{
		...
		if ($project != "0") {
			...
			if (($status != null) && ($status != "")) {
				$filterStatus = "xin_employees.request_paklaring = '" . $status . "'";
			} else {
				$filterStatus = "";
			}
			...
			// AND	pkwt_id = " . $contract_id . "
			// )";
			$kondisiDefaultQuery = "`tx_cio_summary.secid` != '1'";
			...
			$dbtraxes->select('tx_cio_summary.dat_cio');
			$dbtraxes->select('tx_cio_summary.jabatan_name');
			// $this->db->select('xin_designations.designation_id');
			// $this->db->select('xin_designations.designation_name');
			$dbtraxes->select('tx_cio_summary.datetimephone_in');
			//$this->db->select('b.from_date');
			//$this->db->select('b.to_date');
			$dbtraxes->select('tx_cio_summary.datetimephone_out');
			// $this->db->select('tx_cio_summary.contract_end');
			// $this->db->select('tx_cio_summary.private_code');
			// $this->db->select('xin_projects.priority');
			// $this->db->select('xin_designations.designation_name');
			$dbtraxes->where($kondisiDefaultQuery);
			if ($searchQuery != '') {
