...
class Xin_model extends CI_Model
{
	...
	public function select_module_class($mClass, $mMethod)
	{
		...
		if ($mClass == 'dashboard') {
			...
			$arr['adm_open'] = 'open';
			return $arr;
		} else if ($mClass == 'subproject') {
			$arr['sub_project_active'] = 'active';
			$arr['adm_open'] = 'open';
			return $arr;
			...
			return $arr;
			//employe (main)
		} else if ($mClass == 'employees' && $mMethod == 'detail') {
			$arr['myprofile_active'] = 'active';
			$arr['stff_open'] = 'open';
			return $arr;
		} else if ($mClass == 'employees' && $mMethod == 'request') {
			...
			$arr['pkwt_open'] = 'open';
			return $arr;
		} else if ($mClass == 'pkwt' && $mMethod == 'approval') {
			$arr['approval_active'] = 'active';
			$arr['pkwt_open'] = 'open';
			return $arr;
		} else if ($mClass == 'pkwt') {
			$arr['pkwt_active'] = 'active';
			$arr['pkwt_open'] = 'open';
			return $arr;
			...
			$arr['customer_active'] = 'active';
			$arr['customer_open'] = 'open';
			return $arr;
			...
			$arr['my_pkwt_open'] = 'open';
			return $arr;
		} else if ($mClass == 'eslip') {
			...
			$arr['emp_request_open'] = 'open';
			return $arr;
		} else if ($mClass == 'employee_request_tkhl') {
			$arr['emp_request_tkhl_active'] = 'active';
			$arr['emp_request_open'] = 'open';
			return $arr;
			...
			$arr['reports_open'] = 'open';
			return $arr;
		} else if ($mClass == 'karyawan') {
			$arr['karyawan_active'] = 'active';
			$arr['reports_open'] = 'open';
			return $arr;
	...
	// get single employee
	public function read_employee_temp_info($id)
	{

		$sql = 'SELECT * FROM xin_employees_temp WHERE secid = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function read_pkwt_temp_info($id)
	{
		...
		$sql = 'SELECT * FROM xin_employee_contract_temp WHERE secid = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	// get religion user
	public function read_user_xin_ethnicity($id)
	{
	...
	// get marital user
	public function read_user_xin_marital($id)
	{

		$sql = 'SELECT * FROM mt_marital WHERE id_marital = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	// get location user
	public function read_user_xin_office_location($id)
	{
	...
	// get department user
	public function read_user_xin_department($id)
	{
	...
	// get designation user
	public function read_user_xin_designation($id)
	{
	...
	// get designation user
	public function read_user_xin_project($id)
	{
	...
	// get designation user
	public function read_user_bank($id)
	{
	...
	{
		$query = $this->db->query("SELECT * from mt_regencies ORDER BY name ASC");
		return $query->result();
	}

	// get all type user mobile
	public function get_usermobile_type()
	{
		$query = $this->db->query("SELECT * from xin_user_mobile_type");
		return $query->result();
	}
...
public function count_emp_resign()
	...
	public function count_emp_request_hrd($empID)
	{
		...
		$filterProject = "project in (SELECT project_id FROM xin_projects_akses WHERE nip = " . $empID . ")";
	...
	// get pkwt employees	
	public function read_pkwt_employee($id)
	{
...
ORDER BY `next_birthday`");
	...
	public function hrpremium_version()
	{
...
ORDER BY `expiry_date`");
	...
	// get notifications
	public function hrpremium_notifications($module_name, $employee_id)
	{

		$sql = 'SELECT * FROM xin_hrpremium_notificaions WHERE module_name = ? and employee_id = ? and is_notify = ?';
		$binds = array($module_name, $employee_id, 1);
		$query = $this->db->query($sql, $binds);
		return $query->result();
	}
	// get notifications>count
	public function hrpremium_notifications_count($module_name, $employee_id)
	{
	...
	{
		$this->db->insert('xin_hrpremium_notificaions', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function update_notification_record($data, $id, $employee_id, $module_name)
	{
		...
		if ($this->db->update('xin_hrpremium_notificaions', $data)) {
			return true;
		} else {
			return false;
		}
