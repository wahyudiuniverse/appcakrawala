...
class Saltab_model extends CI_Model
{
	...
	//get employee aktif untuk template saltab
	public function get_employee_aktif($project_id, $sub_project_id, $jenis = null)
	{
		$data = array();
		...
		foreach ($records as $row) {
			$status_aktif = "AKTIF";
			if ($row['status_resign'] == "1") {
				$status_aktif = "AKTIF";
			} else if ($row['status_resign'] == "2") {
				$status_aktif = "RESIGN";
			} else if ($row['status_resign'] == "3") {
				$status_aktif = "BLACKLIST";
			} else if ($row['status_resign'] == "4") {
				$status_aktif = "END CONTRACT";
			} else {
			...
			$data[] = array(
				...
				$row['first_name'],
				$row['designation_id'],
				strtoupper($this->Employees_model->get_nama_jabatan($row['designation_id'])),
				"",
				$row['penempatan'],
				$row['region'],
				$this->Employees_model->get_status_kawin($row['marital_status']),
				$row['date_of_joining'],
			);
		}
