...
class Dokumen_skk extends MY_Controller {
						...
						if($resign_date<$lastT){
							...
							if($resign_date<$lastT){	
								...
								if($resign_date<$lastT){	
									...
									$lastT = new DateTime(date("2026-05-24"));
									if($resign_date<$lastT){
										$sign_fullname = 'SISKYLA KHAIRANA PRITIGARINI';
										$sign_nip = '21300033';
	...
	public function view() {
   	...
   	$pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
		...
		// Contoh penggunaan
		// echo "<script type='text/javascript'>alert('$vskk');</script>";
		...
		$eskk = $this->Esign_model->read_skk($vskk);
			...
			if(!is_null($eskk)){
				...
				$nip 							= $eskk[0]->nip;
				$fullname 				= strtoupper($eskk[0]->employee_name);
				$jabatan 					= $eskk[0]->posisi_jabatan;
				$project_name 		= $eskk[0]->project_name;
				$blacklist 				= $eskk[0]->resign_status;
				$jenis_dokumen 		= $eskk[0]->jenis_dokumen;
				...
				$ktp_no 					= $eskk[0]->ktp;
				$address 					= $this->Employees_model->get_alamat_ktp($eskk[0]->nip);
				$createdon 				= $this->Xin_model->tgl_indo(substr($eskk[0]->createdon,0,10));
				$approve_hrd 			= $this->Xin_model->tgl_indo(substr($eskk[0]->approve_hrd_date,0,10));

				if($eskk[0]->sign_company==2){
					$logo_cover = 'tcpdf_logo_sc.png';
					$header_namae = 'PT. Siprama Cakrawala';
				} else if ($eskk[0]->sign_company==3){
				...
				$cetak_error = 'tengah'.$vskk;
			} else {
				// print('cetak atas:'.$vskk);
				// $cetak_error = 'atas'.$vskk;
				// redirect('admin/employee_paklaring_report');
			}
			...
			$pdf->SetTitle('SKK-'.$header_namae.'-'.$nip.'-'.$fullname);
			...
			if($jenis_dokumen	== 2){
				...
				$tbl_2 = '
					...
					<table cellpadding="2" cellspacing="0" border="0" style="text-align: justify; text-justify: inter-word;">
								<tr>
									<td>Depok, '.$approve_hrd.'<br>Yang bertanda tangan di bawah ini,</td>
								</tr>
					</table>
					...
					<table cellpadding="2" cellspacing="0" border="0" style="text-align: justify; text-justify: inter-word;">
								<tr>
									<td>Adalah benar karyawan yang bekerja di Perusahaan kami sejak tanggal <b>'.$join_date.'</b> sampai dengan <b>'.$resign_date.'</b> yang ditempatkan pada <b>'.$project_name.'</b><br> <br>
										Selama bekerja Sdr/i. '.$fullname.' telah menujukan tanggung jawab dan kinerja yang baik dan dengan ini perusahaan mengucapkan terima kasih.<br><br>
					...
					<img src="'.base_url().$qr_code.'" alt="Trulli" width="80" height="80"><br><b><u>'.$sign_fullname.'</u></b></td>
					...
					<p style="font-size: 10px;"><i>* Bahwa dokumen ini sudah ditandatangani secara <b>Elektronik</b> dan dapat dipertanggung jawabkan menurut hukum yang berlaku.<br>* Tandatangan Elektronik yang tercantum secara resmi dikeluarkan oleh '.$header_namae.'.</i></p>';
				...
				redirect('admin/');
				// $cetak_error = 'bawah'.$vskk;
				// print('cetak bawah:'.$vskk);
			}
			...
			ob_start();
			$pdf->Output('skk-'.$ktp_no.'-'.$fullname.'-'.$approve_hrd.'.pdf', 'I');
			ob_end_flush();
