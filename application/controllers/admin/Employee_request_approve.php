...
class Employee_request_approve extends MY_Controller {
	...
	public function index() {
		...
		$data['title'] = $this->lang->line('xin_request_employee_approved').' | '.$this->Xin_model->site_title();
		...
		$data['breadcrumbs'] = $this->lang->line('xin_request_employee_approved');
		$data['path_url'] = 'emp_request_approve';
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('375',$role_resources_ids)) {
			$data['subview'] = $this->load->view("admin/employees/request_approve_list", $data, TRUE);
	...
	public function request_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/employees/request_approve_list", $data);
		} else {
			redirect('admin/');
		}
		...
		$employee = $this->Employees_model->get_employees_request_approve();
