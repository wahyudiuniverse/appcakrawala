...
class Profile extends MY_Controller
{
	...
	public function contract()
	{
		...
		$addendum = $this->Addendum_model->getAddendumbyEmployee($session['user_id']);

		if ($addendum == "") {
		} else {
			foreach ($addendum as $addendum) {
				// designation
				$pkwt = $this->Pkwt_model->read_pkwt_info($addendum['pkwt_id']);
				...
				if (!is_null($subprojects)) {
					$sub_project = $subprojects[0]->sub_project_name;
				} else {
					$sub_project = '--';
				}
				...
				$download =
					'<a href="' . site_url() . 'admin/addendum/cetak/' . $addendum['id'] . '/" target="_blank">
						  <button type="button" class="btn btn-xs btn-outline-twitter">DOWNLOAD</button>
					  </a>';
	...
	//se data modal untuk upload addendum yg sudah ditandatangan
	public function read2()
	{
		...
		$this->load->view('admin/employees/dialog_profile_pkwt2.php', $data);
