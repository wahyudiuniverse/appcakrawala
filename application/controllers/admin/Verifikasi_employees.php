...
class Verifikasi_employees extends MY_Controller {
	...
	public function index() {
		...
		if(empty($session)){ 
			...
			$data['title'] = 'Monitoring Verifikasi Database';
			$data['breadcrumbs'] = 'MONITORING VERIFIKASI KARYAWAN';
			$data['path_url'] = 'emp_view';
			$data['all_projects'] = $this->Project_model->get_project_maping($session['employee_id']);

		if(in_array('110',$role_resources_ids)) {

			$data['subview'] = $this->load->view("admin/employees/monitoring_verifikasi_karyawan", $data, TRUE);
