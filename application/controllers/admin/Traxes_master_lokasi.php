...
class Traxes_master_lokasi extends MY_Controller {
	...
	public function index() {
		...
		if(empty($session)){ 
			...
			$data['title'] = 'Master | Traxes Lokasi/Toko';
			$data['breadcrumbs'] = 'MASTER LOKASI/TOKO';
			$data['path_url'] = 'emp_view';
			$data['all_projects'] = $this->Project_model->get_project_maping($session['employee_id']);

		if(in_array('111',$role_resources_ids)) {

			$data['subview'] = $this->load->view("admin/traxes/master_traxes_lokasi", $data, TRUE);
	...
	public function list_lokasi()
	{
		...
		// Get data
		$data = $this->Traxes_model->get_list_lokasi($postData);
	...
	public function get_data_lokasi()
	{
		...
		// get data diri
		$data = $this->Traxes_model->get_data_customer($datarequest);

		if (empty($data)) {
			$response = array(
				'status'	=> "201",
				'pesan' 	=> "Lokasi/Toko tidak ditemukan",
			);
		} else {
