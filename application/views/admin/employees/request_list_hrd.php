															...
															<td style='width:100%'>
																<div class="row">
																	<div class="col-md-6 mt-2">
																		...
																		<font color="#FF0000"><i> * Hanya Lihat</i></font>
	...
	<div class="col-md-12">
		...
		<script>
			function cek_rekening_single() {
				...
				//-------cek apakah ada yang tidak diisi-------
				var pesan_bank_verifikasi_modal = "";
				var pesan_norek_verifikasi_modal = "";
				var pesan_pemilik_rekening_verifikasi_modal = "";
				if ((pemilik_rekening_input == "") || (pemilik_rekening_input == null)) {
					pesan_pemilik_rekening_verifikasi_modal = "<small style='color:#FF0000;'>Pemilik rekening tidak boleh kosong</small>";
					// $('#saltab_to').focus();
				}
				if ((norek_input == "") || (norek_input == null)) {
					pesan_norek_verifikasi_modal = "<small style='color:#FF0000;'>Nomor rekening tidak boleh kosong</small>";
					// $('#saltab_from').focus();
				}
				if ((bank_input == "") || (bank_input == null)) {
					pesan_bank_verifikasi_modal = "<small style='color:#FF0000;'>Bank tidak boleh kosong</small>";
					// $('#periode_salary').focus();
				}
				$('#pesan_bank_verifikasi_modal').html(pesan_bank_verifikasi_modal);
				$('#pesan_norek_verifikasi_modal').html(pesan_norek_verifikasi_modal);
				$('#pesan_pemilik_rekening_verifikasi_modal').html(pesan_pemilik_rekening_verifikasi_modal);

				//-------action-------
				if (
					(pesan_bank_verifikasi_modal != "") || (pesan_norek_verifikasi_modal != "") || (pesan_pemilik_rekening_verifikasi_modal != "")
				) { //kalau ada input kosong 
					// alert("Tidak boleh ada input kosong");
				} else {
					//action cek rekening
					$.ajax({
						url: '<?= base_url() ?>admin/Importexcel/tes_API_Bank3_only/',
						method: 'POST',
						data: {
							[csrfName]: csrfHash,
							id_bank: bank_input,
							norek: norek_input,
							pemilik_rekening: pemilik_rekening_input,
						},
						beforeSend: function() {
