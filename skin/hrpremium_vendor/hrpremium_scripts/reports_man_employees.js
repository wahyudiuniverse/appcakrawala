$(document).ready(function() {
   var xin_table = $('#xin_table').dataTable({
        "bDestroy": true,
		"ajax": {
            url : site_url+"reports/report_manage_employees_list/0/0/0/",
            type : 'GET'
        },
		dom: 'lBfrtip',
	...
	$("#employee_reports").submit(function(e){
		...
		var xin_table2 = $('#xin_table').dataTable({
			"bDestroy": true,
			"ajax": {
				url : site_url+"reports/report_manage_employees_list/"+company_id+"/"+department_id+"/"+project_id+"/"+subproject_id+"/",
				type : 'GET'
			},
			dom: 'lBfrtip',
			"buttons": ['csv', 'excel', 'pdf', 'print'], // colvis > if needed
