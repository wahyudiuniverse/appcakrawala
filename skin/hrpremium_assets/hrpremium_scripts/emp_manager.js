...
//$('#hrload-img').hide();
	...
	var obj = $(this), action = obj.attr('name');
	fd.append("is_ajax", 1);
	fd.append("type", 'document_id');
	fd.append("data", 'document_id');
	fd.append("form", action);
	e.preventDefault();
	$('.icon-spinner3').show();
	$('.save').prop('disabled', true);
...
//$('#hrload-img').hide();
	...
	// On page load > xin_hrpremium_table
	
	$('.xin_hrpremium_table').DataTable();
	/* Add contact info */
...
//$('#hrload-img').hide();
	...
	// 		type: "POST",
	// 		url: e.target.action,
	// 		data: obj.serialize()+"&is_ajax=1&data=document_info&type=document_info&form="+action,
	// 		cache: false,
	// 		success: function (JSON) {
	// 			if (JSON.error != '') {
	// 				toastr.error(JSON.error);
	...
	$("#document_pkwt").submit(function(e){
		...
		var obj = $(this), action = obj.attr('name');
		fd.append("is_ajax", 7);
		fd.append("type", 'document_pkwt');
		fd.append("data", 'document_pkwt');
		fd.append("form", action);
		e.preventDefault();
		$('.icon-spinner3').show();
		$('.save').prop('disabled', true);
...
//$('#hrload-img').hide();
					...
					$('.icon-spinner3').hide();
					jQuery('#document_pkwt')[0].reset(); // To reset form fields
					$('.save').prop('disabled', false);
				}
			},
			error: function() 
...
// 	});
	...
	/* Add bank account info */
	jQuery("#salary_account_info").submit(function(e){
	/*Form Submit*/
	e.preventDefault();
		var obj = jQuery(this), action = obj.attr('name');
		jQuery('.save').prop('disabled', true);
...
//toastr.info(processing_request);
		jQuery.ajax({
			type: "POST",
			url: e.target.action,
			data: obj.serialize()+"&is_ajax=16&data=salary_account_info&type=salary_account_info&form="+action,
			cache: false,
			success: function (JSON) {
				if (JSON.error != '') {
					//toastr.clear();
...
//$('#hrload-img').hide();
					...
					$('.icon-spinner3').hide();
					jQuery('#salary_account_info')[0].reset(); // To reset form fields
					jQuery('.save').prop('disabled', false);
				}
			}
		});
